      ...
      <nav className="flex max-h-36 gap-1 overflow-x-auto md:max-h-none md:flex-1 md:flex-col md:overflow-y-auto">
        {sessions.map((session, index) => {
          ...
          return <div key={sessionId || `session-${index}`} className={`group flex min-w-40 items-center rounded-lg ${activeSessionId === sessionId ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-900 hover:text-slate-200'}`}>
            <button onClick={() => selectSession(sessionId)} className="min-w-0 flex-1 truncate px-3 py-2 text-left text-sm">{session.title || 'New chat'}</button>
            <button onClick={() => deleteSession(sessionId)} className="mr-1 rounded p-1.5 opacity-60 hover:bg-slate-700 hover:text-rose-300 md:opacity-0 md:group-hover:opacity-100" title="Delete chat"><Trash2 size={15} /></button>
          </div>
        })}
      </nav>
