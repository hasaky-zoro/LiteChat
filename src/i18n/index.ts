...
const messages = {
  'zh-CN': {
    ...
    providersAndModels: '模型供应商', providerDescription: '从预设快速添加供应商，或探测可用模型并验证模型响应。', providerName: '供应商名称', removeProvider: '删除供应商', baseUrl: '基础 URL', apiKey: 'API 密钥', apiKeyHint: 'sk-...（本地供应商无需填写）', models: '模型', modelsAvailable: '（{count} 个可用）', fetchModels: '探测模型', fetchingModels: '正在探测模型…', modelsInput: '{provider} 的模型，每行一个', modelToTest: '要测试的模型', selectModel: '选择模型', testConnection: '测试连通性', testing: '测试中…', providerPreset: '供应商预设', openaiCompatibleProvider: 'OpenAI 兼容接口（自定义供应商）', addProvider: '添加供应商', save: '保存', saveSettings: '保存设置', chatSettings: '对话设置', systemPrompt: '系统提示词', systemPromptHint: '为当前对话设置模型行为。', temperature: '随机性 (Temperature)', temperatureHint: '较低的值更稳定，较高的值更有创造性。',
